...
def bbring_profile(rin, rout, mstar, inc, res=2., passband=100., nr=10., nphi=50.):
    '''                                                                                                            
    Create a line profile for a blackbody ring with given inner and outer radius, inclination.  Assumes single temperature
    ...
    inc : float
          inclination of disk                     
    res : float, optional
          velocity binsize, in km/s
    passband : float, optional
          max velocity for computing line profile, in km/s
    nr : float, optional
    ...
    --------                                                                                                       
    profile : dictionary
          contains 'velocity' and 'flux'
    '''

    redges=np.linspace(rin,rout,nr)
    areas=np.zeros(np.size(redges)-1) 
    for i,myr in enumerate(redges[:-1]):
        areas[i]=np.pi*redges[i+1]**2. - np.pi*redges[i]**2.



    phicen=np.linspace(0,2*np.pi,np.int(nphi+1))[:-1]
...
def spec_convol(wave, flux, dv, kernel=None):
    ...
    if(kernel is not None):
        dv=np.max(kernel['vel'])

    dv=fwhm_to_sigma(dv)  #Convert FWHM to sigma

    n=round(4.*dv/(c.value*1e-3)*np.median(wave)/(wave[1]-wave[0]))   #Determine # of elements to use in kernel. Looks like 4 pix per sigma.
    if (n < 20):  #Set floor of size of kernel.  Actually has 2n+1 elements.
        n=20.
...
#Pad arrays to deal with edges
    ...
    for i in np.arange(n, np.size(wave)-n+1):
        lwave=wave[np.int(i-n):np.int(i+n+1)]  #Local wavelengths
        lflux=flux[np.int(i-n):np.int(i+n+1)]  #Local fluxes
        lvel=(lwave-wave[np.int(i)])/wave[np.int(i)]*c.value*1e-3 #Local velocities
        if(kernel is None):
#            nvel=(np.max(lvel)-np.min(lvel))/(dv*0.2) +3               #
#            vel=np.arange(nvel)
#            vel=0.2*dv*(vel-np.median(vel))  #Velocities on which to compute the well-sampled kernel values
#            mykernel=markgauss(vel,mean=0,sigma=dv,area=1.)
#            wkernel=np.interp(lvel,vel,mykernel)   #interpolate well-sampled kernel onto the local velocities
            wkernel=markgauss(lvel,mean=0,sigma=dv,area=1.)  #Compute kernel values on local velocities
        if(kernel is not None):
            wkernel=np.interp(lvel,kernel['vel'],kernel['flux'])   #interpolate kernel onto the local velocities
